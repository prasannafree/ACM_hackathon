...
class ResNet(nn.Module):
    def __init__(self, device="cpu", args: dict = None):
        ...
        if layers is None:
            depth = args.get("depth", 101)
            if depth not in DEPTH_TO_LAYERS:
                raise ValueError(f"Unsupported depth {depth}; choose from {list(DEPTH_TO_LAYERS)}")
            layers = DEPTH_TO_LAYERS[depth]

        groups = args.get("groups", 1)
        width_per_group = args.get("width_per_group", 64)
        norm_layer = nn.BatchNorm2d
        ...
        if args.get("zero_init_residual", True):
            for m in self.modules():
...
if __name__ == "__main__":
    net = ResNet(args={"num_classes": 10, "depth": 101})
